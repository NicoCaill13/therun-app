...
## Upcoming Runs — Mobile screen (see `code.html`)

Platform-specific layout tokens that **differ from desktop** for this route:

*   **Column:** Single narrow feed (`max-w-lg`), content aligned start (editorial left stack), **not** a multi-column grid.
*   **Hero title:** Two lines at **~3.5rem** scale: line 1 `UPCOMING` on `on_surface` (#ffffff); line 2 `RUNS` on **`primary_fixed_dim`** (#ff5d2b) — not the same color treatment as desktop hero.
*   **Kicker:** Technical label row under hero — e.g. `URBAN KINETICISM / EXPLORE PHASE` in `on_surface_variant`, wide tracking, uppercase (Label-MD feel).
*   **Create Event:** **Floating** primary control (icon-first), fixed toward **bottom-end** of the viewport (above bottom nav / safe area), **not** the wide inline hero CTA used on desktop.
*   **Cards:** Uniform **stacked** feed cards (`surface_container_low`, 4px primary accent); same card pattern repeated with **16px** vertical rhythm — no featured “hero card + rail” split.
*   **Top bar:** Compact app bar (avatar + mark + notifications) consistent with small viewports; differs from desktop marketing-style top nav density.